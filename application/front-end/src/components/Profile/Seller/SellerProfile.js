...
class SellerProfile extends Component {
  ...
  getProfile = () => {
    let sellerid = this.props.location.state.sellerid;
    axios.all([
      axios.get(`http://34.94.123.246:5000/user/?getUserbyUserid=${sellerid}`),
      axios.get(`http://34.94.123.246:5000/product/?getAllProductbyUserid=${sellerid}`)
    ])
  ...
  viewProfile = (user, product) => {
    ...
    if (product.data) {
      ...
      prodLength = 0;
      tempProduct = <h3>How do you even get hear?</h3>
    }
    const tempView =
      <>
        <Container fluid>
  ...
  getTempProfile = () => {
    ...
    this.viewProfile(user.data, product.data)
  }
